DROP DATABASE kopetrart_rh;

CREATE DATABASE kopetrart_rh;
...
CREATE TABLE dossiers (
    ...
    progression_status VARCHAR(50) NOT NULL DEFAULT 'En Attente',
    progression NUMERIC(3) NOT NULL DEFAULT 0
);
...
CREATE TABLE test(
    ...
    is_qna BOOLEAN NOT NULL DEFAULT FALSE,
    file VARCHAR(100)
);
...
CREATE TABLE notification_seen(
    id SERIAL PRIMARY KEY,
    id_notification INT NOT NULL REFERENCES notification(id) ON DELETE CASCADE,
    id_admin INT NOT NULL REFERENCES admin(id_admin) ON DELETE CASCADE,
